...
import {NativeRouter, Link, Route, Routes as Router, redirect} from 'react-router-native';
...
const LoginForm = () => {
    ...
    const onSubmitFormHandler = async (event) => {
      ...
      try {
        const response = await axios.post(`${baseURL}/connection`, { 'email':email, 'password':password }, { headers: {'content-type': 'application/json'}});
        if (response.status === 200) {
          alert(` You are connected: ${JSON.stringify(response.data)}`);
          setEmail('');
          setPassword('');
          setLoggedIn(false)
